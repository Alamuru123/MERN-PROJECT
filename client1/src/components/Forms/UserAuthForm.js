...
const LoginForm = () => {
  ...
  const validationLoginForm = Yup.object().shape({
    phoneLogin: Yup.string().phone('US', false, 'Phone number is invalid')
      .required(),
  ...
  const validationRegisterForm = Yup.object().shape({
    ...
    phoneRegister: Yup.string().phone('US', false, 'Phone number is invalid')
      .required(),
  ...
  const onLogin = async (data) => {
    ...
    await axios.post('http://localhost:8000/user/login', data)
      .then(res => {
        ...
        console.error(error);
        alert('Something went wrongdfgh. Please try again.');
      })
  };
